...
(function () {
  ...
  function load() {
    if (loading) return;
    // DEMO MODE (tour): build the timeline from sample data, never touch Supabase.
    if (window.QWDemo && QWDemo.isOn()) {
      srcState = { quotes: "ok", resolutions: "ok", gaps: "ok" };
      events = buildEvents(QWDemo.quotes(), QWDemo.resolutions(), QWDemo.gaps());
      loaded = true; loading = false; render(); return;
    }
    loading = true;
